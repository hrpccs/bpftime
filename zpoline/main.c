...
// 			-1, 0)) != MAP_FAILED);
// #endif
// 	printf("try setup_trampoline\n");
// 	setup_trampoline();
// 	printf("try rewrite_code\n");
// 	rewrite_code();
// 	printf("try load_hook_lib\n");
// 	load_hook_lib();
// 	printf("try load_hook_lib done\n");
// }

void bpftime_agent_main(const char *data  __attribute__((unused)), int *stay_resident)
{
	...
	*stay_resident=true;
	...
	assert((bm_mem = mmap(NULL, BM_SIZE,
			...
			-1, 0)) != MAP_FAILED);
#endif
	printf("try setup_trampoline in bpftime agent main\n");
	setup_trampoline();
	printf("try rewrite_code\n");
	rewrite_code();
	printf("try load_hook_lib\n");
